...
def load_verification_dataset():
    ...
    if isinstance(data_config, dict):
        ...
        if len(data_config) == 5:
            ...
            # import numpy as np
            # with open('/home/users/nus/dcsv206/inst.npy', 'rb') as fp:
            #     X = np.load(fp)
            #     labels = np.load(fp)
            #     X = torch.tensor(X)
...
def parse_run_mode(verb = 0):
    ...
    if (arguments.Config['general']['csv_name']
        ...
        if arguments.Config['specification']['vnnlib_path'] is None:
            ...
            # with open('abcrown_vnnlib.txt', 'w') as f:
            # import pickle
            # with open('abcrown_vnnlib.pkl', 'wb') as f:
            #     pickle.dump(vnnlib_all[0], f)
