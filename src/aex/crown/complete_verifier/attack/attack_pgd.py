...
def attack(model_ori, x, vnnlib, verified_status, verified_success,
    ...
    if 'auto_attack' not in arguments.Config["attack"]["attack_mode"]:
        ...
        #print(data_max_repeat - data_min_repeat == 0.00)
        #print( (data_max_repeat - 0.001) == (data_min_repeat + 0.001) )
        # print(data_max_repeat.shape, x.shape )
    ...
    return verified_status, verified_success, attack_images, attack_margins, all_adv_candidates # Mahi: it should be clear
...
def pgd_attack_with_general_specs(model, X, data_min, data_max, C_mat, rhs_mat,
                                  ...
                                  return_early_stopped=False, verb = False):
...
def attack_pgd(model, X, y, epsilon, alpha, attack_iters, num_restarts,
        ...
        initialization='uniform', GAMA_loss=False, nn4sys=False, verb = False):
    if initialization == 'osi':
        if multi_targeted:
...
def attack_with_general_specs(model, x, data_min, data_max,
                              list_target_label_arrays,
                              initialization="uniform", GAMA_loss=False, verb = False):
    ...
    best_deltas = None
    best_loss = None
    for _ in tqdm(range((num_restarts + batch_size - 1) // batch_size),disable=True):
