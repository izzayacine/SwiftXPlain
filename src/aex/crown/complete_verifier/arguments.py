...
class ConfigHandler:
    ...
    def add_common_options(self):
        ...
        h = ["general"]
        MAX_NUM_GPUS = 100 # torch.cuda.device_count()
        gpus = [f"cuda:{gpu_id}" for gpu_id in range(MAX_NUM_GPUS)]
        cpus = [f"cpu:{gpu_id}" for gpu_id in range(MAX_NUM_GPUS)]
        self.add_argument("--device", type=str, default="cuda", choices=["cpu", "cuda"]+gpus+cpus,
